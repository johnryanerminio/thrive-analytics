...
def _generate_static_index(out: Path):
    ...
    async _loadMasterRange(tab) {
      ...
      // Analytical tabs (retention, forecast, products, waterfall, basket, heatmap)
      // need full historical context — fall back to all-time file for ranges
      const analyticalTabs = ['retention','forecast','products','waterfall','basket','heatmap'];
      if (analyticalTabs.includes(tab)) {
        const fb = await this._fetchJson('/data/master/' + tab + '.json');
        if (fb) return fb;
      }
